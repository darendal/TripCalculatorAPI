...
namespace TripCalculatorAPI.Controllers
{
    public class TripCalculatorController : ApiController
    {
        ...
        //    {
        //        //empty result passed
        //        ThrowResponseException(HttpStatusCode.BadRequest, "Empty list of expenses entered");
        //    }

        //    if(expenses.Any(exp => exp.ExpenseAmount < 0))
        //    {
        ...
        //    var totalExpenses = expenses.GroupBy(exp => exp.Name)
        //                        .Select(x => new ExpenseLineItem() { Name = x.Key, ExpenseAmount = x.Sum(y => y.ExpenseAmount)}).ToList();
        ...
        public ExpenseRepaymentCollection Post(IEnumerable<User> users)
        {
            ...
            //Flatten list into 1 expense line item per name, with their total expenditure
            var totalExpenses = 
                users.Select(x => new ExpenseLineItem() { Name = x.Name, ExpenseAmount = x.Expenses.Sum()}).ToList();
